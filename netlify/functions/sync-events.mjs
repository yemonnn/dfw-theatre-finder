...
export default async () => {
  ...
  const showLinks = Array.from(html.matchAll(/regionalshows\.cfm\?showid=\d+/g))
    .slice(0, 25)
    .map((m) => m[0]);

  // Pull URLs that look like feeds/APIs/json/etc
  const apiHints = Array.from(html.matchAll(/https?:\/\/[^"' ]+/gi))
    .map((m) => m[0])
    .filter((u) => /feed|rss|json|api|regional|dallas|shows|calendar|events|graphql|wp-json/i.test(u))
    .slice(0, 50);

  // Pull script src URLs (often where API endpoints are hidden)
